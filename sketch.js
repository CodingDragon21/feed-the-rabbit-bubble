...
function setup() {
  ...
  shelf = new Ground(600,200, 100,10)

  blink.frameDelay = 20;
  eat.frameDelay = 20;

  bunny = createSprite(600,130,100,100);
  bunny.scale = 0.2;
...
function draw() 
{
 ...
 /* push()
  ...
  {
     fruit.position.x = bubble.x
     fruit.position.y = bubble.y
  }

  if(fruit.position.x === bubble.position.x &&  fruit.position.y === bubble.position.y ){
   force()
  }
...
function force(){
 ...
 // Matter.Body.applyForce(bubble,{x:0,y:0},{x:0,y:-0.02})
}
